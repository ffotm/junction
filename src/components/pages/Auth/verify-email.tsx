...
export default function VerifyEmailPage() {
  ...
  function handleResend() {
    setResent(true);
    // Here you would trigger resend logic
  }
    ...
    <div className="min-h-screen flex flex-col justify-center items-center bg-gradient-to-br from-[#1e3a8a] via-[#3b82f6] to-[#ffffff]">
      <div className="w-full max-w-4xl h-auto bg-white/10 rounded-3xl shadow-2xl flex flex-col md:flex-row overflow-hidden relative">
        ...
        <div className="relative z-10 flex items-center justify-center min-w-[60%] bg-white/80 backdrop-blur-md p-8 md:p-12">
          <Card className="w-full max-w-lg md:max-w-xl shadow-lg rounded-2xl border-0">
            <CardContent className="py-10 px-8 flex flex-col items-center">
              <h3 className="text-2xl font-bold text-[#1e3a8a] mb-6 text-center">Verify your email</h3>
              <p className="text-center text-gray-700 mb-6">
                We&apos;ve sent a verification link to your email address.<br />
                Please check your inbox and follow the instructions to verify your account.
              </p>
              {resent && (
                <div className="text-green-700 font-medium mb-4">Verification email resent!</div>
              )}
              <Button onClick={handleResend} className="w-full mb-4 bg-gradient-to-r from-[#1e3a8a] to-[#3b82f6] text-white font-semibold text-lg rounded-lg shadow-md hover:from-[#3b82f6] hover:to-[#1e3a8a] transition-all duration-200">
                Resend Verification Email
              </Button>
              <Link href="/login" className="text-[#1e3a8a] hover:underline hover:text-[#3b82f6] transition-colors text-sm">Back to Login</Link>
            </CardContent>
          </Card>
        </div>
        {/* Right: Info content */}
        <div className="relative z-10 flex-1 flex flex-col justify-center items-end px-10 py-12 bg-gradient-to-br from-[#1e3a8a]/90 to-[#3b82f6]/80 text-white min-w-[35%] text-right">
          <h1 className="text-4xl md:text-5xl font-extrabold mb-4 drop-shadow-lg">Email Verification</h1>
          <h2 className="text-xl md:text-2xl font-semibold mb-6">Almost there!</h2>
          <p className="text-lg mb-8 max-w-md opacity-90 ml-auto">
            To complete your registration, please verify your email address. Didn&apos;t get the email? You can resend it.
          </p>
